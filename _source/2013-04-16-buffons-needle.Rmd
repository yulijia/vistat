...
tags: [Buffon's needle, Monte Carlo method, Pi]
reviewer: [yihui]
animation: true
---
{% include JB/setup %}
...
approximating the number [$$\pi$$](http://en.wikipedia.org/wiki/Pi).

In the [**animation** package](http://yihui.name/animation), functions `buffon.needle()` can be
used to produce simulation of Buffon's needle. There are three graphs made in each step: the top
one is a simulation of the scenario, the bottom-left one can help us understand the connection
between dropping needles and the mathematical method to estimate $$\pi$$, and the bottom-right one
is the simulation result $$\pi$$ for each drop.

```{r buffon-needle, fig.show='animate', interval=0.5, fig.width=8, fig.height=8}
library(animation)
ani.options(nmax = 150, interval = 0.5)
par(mar = c(3, 2.5, 0.5, 0.2), pch = 20, mgp = c(1.5, 0.5, 0))
buffon.needle(mat=matrix(c(1,2,1,3),2), heights=c(1,1))
```
