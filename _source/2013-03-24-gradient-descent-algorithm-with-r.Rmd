---
layout: post
title: "Gradient Descent Algorithm with R"
author: [Lijia Yu]
categories: [Animation, CompStat]
...
On [**animation** package](http://yihui.name/animation), there is a function named `grad.desc()`. It provids a visual illustration for the process of minimizing a real-valued function through [Gradient Descent Algorithm](http://en.wikipedia.org/wiki/Gradient_descent). These two examples show you how to use `grad.desc()` function.
...
```{r grad.desc-right, fig.show='animate', interval=.2, fig.height=5}
library(animation)
ani.options(nmax = 50)    
grad.desc()
```
...
```{r grad.desc-wrong, fig.show='animate', interval=.2, fig.height=5}
ani.options(nmax = 70)
