...
animation: true
---
{% include JB/setup %} In [Probability Theory](http://en.wikipedia.org/wiki/Probability_theory),
the [Central Limit Theorem (CLT)](http://en.wikipedia.org/wiki/Central_limit_theorem) states that,
given certain conditions, the mean of a sufficiently large number of independent random variables,
each with a well-defined mean and well-defined variance, will be approximately normally
distributed.
...
```{r central-limit-theorem, fig.show='animate', interval=.5, fig.height=8, fig.width=8}
library(animation)
